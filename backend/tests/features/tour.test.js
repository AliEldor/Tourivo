...
describe("Tour API", () => {
  ...
  describe("GET /api/v1/tours", () => {
    ...
    it("should get tours by search criteria", async () => {
      ...
      const foundTour = response.body.data.find(
        (t) => t.city === "SearchTestCity"
      );
    ...
    it("should get featured tours", async () => {
      ...
      const response = await request(app).get(
        "/api/v1/tours/search/getFeaturedTours"
      );
  ...
  describe("GET /api/v1/tours/:id", () => {
    ...
    it("should get a single tour by ID", async () => {
      const response = await request(app).get(`/api/v1/tours/${testTour._id}`);
    ...
    it("should return 404 for non-existent tour ID", async () => {
      ...
      const response = await request(app).get(`/api/v1/tours/${nonExistentId}`);
  ...
  describe("DELETE /api/v1/tours/:id", () => {
    ...
    it("should delete a tour when admin is authenticated", async () => {
      ...
      } else if (response.body.data && response.body.data.message) {
        expect(response.body.data.message).toBe("Successfully deleted");
      }
